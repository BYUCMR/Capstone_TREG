...
%Now try turning off some rollers? 
%CanI walk another way? 
%Now solving correctly...
...
%% %Rotate so that 1 2 7 9 are on the ground

R = alignPlaneToXY(x_all(1,:), x_all(2,:), x_all(7,:))

x_all_n= (R* x_all')'

x_all=x_all_n
node_origin= 9
x_all(:,1)= x_all(:,1)- x_all(node_origin,1)
x_all(:,2)= x_all(:,2)- x_all(node_origin,2)
x_all(:,3)= x_all(:,3)- x_all(node_origin,3)


th_z= atan2(x_all(1,2), x_all(1, 1))
...
%% Drive some of the edges to make it stand up properly? 



%% Define Contact with the Environment
Support=[1 2 7]; %The Support Polygon Base
...
Support=[1, 2, 9];   %9 is th eother constraint node, but I am locking it here
...
C_new=C_Lock([1:3, 6, 9, 7, 8],:)
C_Lock = C_new;
C_Extra= zeros(1, 3*n_all);
C_Extra(7+2*n_all)=1; %Lock 7 in z

C_Lock=[C_Lock; C_Extra]


%Try another locking set? 
C= zeros(9,3*n_all)
...
%Fix node 9 in x and y
C(5,9)=1;
C(6,9+n_all)=1;

C(7,7)=1; %Fix node 7 in x

%Fix nods 1 and 2 to have the same x value
C(8,1)=1;
C(8,2)=-1;
C(9,1+n_all)=1
C_Lock = C;
...
Nodes_Move=[3]; %I could add more edges, or jsut adjust this...
...
b_con= [0; 0; 1]; %repmat([0; 0; -1],[3 1]);

A_move{1}=Move_Con(3,:);
A_lock{1}=C_Lock;
b_move{1}=1;  %Used to be b_con
b_lock{1}=zeros(size(A_lock{1},1),1);
t_move{1}=1;

%% Move 2- Step up foot 1
Support=[9, 7, 2];
C_Lock=zeros(length(Support)*3,3*n_all);
for i=1:length(Support)
        C_Lock(3*(i-1)+1,Support(i))=1;
...
Nodes_Move=[1]; %I could add more edges, or jsut adjust this...
Move_Con=zeros(3*length(Nodes_Move),3*n_all);
for i=1:length(Nodes_Move)
        Move_Con(3*(i-1)+1,Nodes_Move(i))=1;
        Move_Con(3*(i-1)+2,Nodes_Move(i)+n_all)=1;
...
% Foot_Move= [1, 7, 9, 2]; %An index of the four feet
Foot_Move= [1, 9, 2, 7]; %Alternating the side of moving the foot
Ind_foot=1; %Start on the first foot
%Build the constraint matrix for all feet
C_feet=zeros(4*3,3*n_all);
for i=1:length(Foot_Move)
        C_feet(3*(i-1)+1,Foot_Move(i))=1;
        C_feet(3*(i-1)+2,Foot_Move(i)+n_all)=1;
        C_feet(3*(i-1)+3,Foot_Move(i)+2*n_all)=1;
end


N_Moves=2
t_move{2}=3; %Do the steps for 3 seconds
t_step=.25;  %The time for each step
t_switch=0; 
...
%             Turn_On_Broadcast=data.broadcast_command.Value;
        %     Node_Control=0;  %For now Hardcoded (picks the node, or the center of mass)
            if Movement_i==1  %For the first movement, follow this pre-prgrammed step
                A=[ A_move{Movement_i}; A_lock{Movement_i}];  
                b=[ b_move{Movement_i}; b_lock{Movement_i}];
            else   %Instead, determine which foot to move, and what command to give
                time= it_t* dt; %How long has this move been going 
                Current_Foot= Foot_Move(Ind_foot)
                %Determine the Specified Direction of Motion
                 if (it_t* dt- t_switch) >t_step/2
                     b_move= [0; 1; -1]; %Move downward
                 else 
                     b_move= [0; 1; 1];  %Move Upward
                 end
                 A=C_feet;
                 b=zeros(4*3,1);
                 b(3*(Ind_foot-1)+1:3*Ind_foot)=b_move;
                 if time-t_switch>t_step
                    t_switch= time;
                    Ind_foot= Ind_foot+1;
                    if Ind_foot>4  %Reset to the first foot if they have all moved
                        Ind_foot=1;
                    end
                 end
...
count=1;
for i_mov=1:length(Frames) %:length(Frames)
        clf
        ...
        axis off
        axis equal
        axis([xmin,xmax,ymin,ymax,zmin,zmax+1])
        Ground=[xmin ymin; xmax ymin; xmax ymax; xmin ymax];
        patch(Ground(:,1), Ground(:,2),-.01*ones(length( Ground(:,2)),1),[.9 .9 .9]);
...
SAVEMOVIE=1;
if SAVEMOVIE==1
    v = VideoWriter('Walking_Worm_alt');
...
%% Visualize how much each of the individual rollers move to do this. 
