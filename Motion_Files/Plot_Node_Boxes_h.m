function [  ] = Plot_Node_Boxes_h( x_vec_temp, T2T_Qual, d_up, d_down, d_length, d_width, Passive,  handle )
...
%         T2T_Angle_2(i,j)=acos(Tube1_Vec*Tube2_Vec'/(norm(Tube1_Vec)*norm(Tube2_Vec)));
    ...
    color=[1 0 0];
    color_p=[0 1 0]; %The color of the passive nodes
        if ismember(T2T_Qual(j,1),Passive)
           patch(handle, 'faces',faces,'vertices',vertices,'facecolor',color_p,'facealpha',.1, 'edgecolor','k','linewidth', 1);
        else
           patch(handle, 'faces',faces,'vertices',vertices,'facecolor',color,'facealpha',.1, 'edgecolor','k','linewidth', 1);
        end
     hold on
    end
