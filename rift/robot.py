...
def rotate_points_about_line(points, line_point, line_dir, angle):
    """
    Rotate a list of 3D points around a line defined by a point and direction vector.

    Args:
        points: (N,3) array-like — points to rotate
        line_point: (3,) array-like — a point on the rotation axis
        line_dir: (3,) array-like — direction vector of the rotation axis
        angle: float — rotation angle in radians (positive = right-hand rule)

    Returns:
        np.ndarray — rotated points, shape (N,3)
    """
