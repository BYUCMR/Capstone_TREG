...
class Path():
    def __init__(self, type: str, dimension: int, ZYZrot: list[float], start_coords: np.ndarray, length: float = 1, num_sides: int = 4) -> None:
        options = {
        ...
        ZYZrot = [np.deg2rad(angle) for angle in ZYZrot]
        
        self.raw_path: np.ndarray = options[type](length, num_sides)
        self.transformed_path: np.ndarray = self._calc_transformed_path(ZYZrot, start_coords)
    ...
    def _calc_transformed_path(self, ZYZrot, start_coords) -> np.ndarray:
        
        if self.d == 3:
            self.R = rotz(ZYZrot[0])@roty(ZYZrot[1])@rotz(ZYZrot[2])
        else:
            self.R = rot2D(ZYZrot[0])
