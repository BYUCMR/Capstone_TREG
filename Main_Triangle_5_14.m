...
%     Node_Control=0;  %For now Hardcoded (picks the node, or the center of mass)
    ...
    if JOYSTICK_present==1
        ...
        Period=5;
        Osc=.1*sin(2*pi*t_vec(end)/Period);
...
[th_int,Val]=fminunc(Obj, ones(3,1)*30*pi/180);
Angle=th_int*180/pi;
...
%Use the relative Positions to extract these things
...
Look_Step=.1; %How far to propoate the command to send a position command
d_Tube_Position_m(count+1,:)=d_Tube_Position_m(count,:)+Command'*Look_Step;
...
%         Plot_Robot_Edges_Color_Spec( Edges_Test(1:12,:), reshape(y_tot(i,:)',n_all,3), .05, [1 0 0] )
    ...
    if Turn_On_Broadcast==1
       ...
       setappdata(0,'Command_from_controller',d_Tube_Position_m(count+1,:));
