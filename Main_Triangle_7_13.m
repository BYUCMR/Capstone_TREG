...
%% Reconstruct the Shape using the optimization
%Reconstruct the shape by exchanging the graduent information

%Fix The correct points
...
Dist_Est=1; %For now, perform the distributed updates
if Dist_Est==1
    
    %Obtain the Physical Measurements from the Robot
...
%     L_perim=sum(Lengths_All(1:3));
    L_meas_All=Lengths_All;
    L_meas_All(1:3)=L_meas; %Insert the first measured lengths
    for i=1:N_robots
    %     Inds_Rel=find(Edges_All(:,1)==i); %Find the incident edges
    %     Cost{i}= @(x) Cost_Dist(x, Edges_All(Inds_Rel,:), L_noise(Inds_Rel));
        Cost{i}= @(x) Cost_Dist(x, Edges_All(Edges_Local{i},:), Edges_All(Edges_All_Inds,:), L_meas_All(Edges_Local{i}), L_perim, Order(i,:));
    end
    ...
    A_local{1}(1,Foot_1)=1;
    A_local{1}(2,n_all+Foot_1)=1;
    A_local{2}=zeros(1,2*n_all);
    A_local{2}(1,Foot_2+n_all)=1; %Constraint in Z
    A_local{3}=[];
    ...
    %Doing this just so it isn't empty
    A_local{3}=A_local{2};
    b_local{3}=b_local{2};
...
%     N_robots=n
    ...
    x_ADMM(:,:,1)=repmat(y_tot(end,1:2*n_all)',1,N_robots); %Injecting noise into this initial guess
    ...
    % Duration_solve=toc
        ...
        x_recon_switch=[reshape(x_ADMM(:,1,end),n_all,2), zeros(n_all,1)];
...
if Cent_Decentralized_On==1
    ...
    %Use the latest Update to change the shape
    [ x_opt, Ldot ] = Controled_Motion_Nodes_Dist_Any(Recon(count,:), Edges_All, Edges_All_Tube, Loop_Con, L2th, Order, A, b, L_norm, Cost_Fun_Index, L_robots, Local, Passive);
