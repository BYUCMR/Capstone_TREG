...
Support=[1 2 3]; %The Support Polygon Base
...
%     Node_Control=0;  %For now Hardcoded (picks the node, or the center of mass)
    ...
    if JOYSTICK_present==1
        ...
        Command_Normalized=[1 0 0]'*Max_Speed_Node;
%     else
%          keyboardstate= read(kb);
...
Command_Normalized=Command_Normalized(1:3); %Take the first two rows if just doing xy control
