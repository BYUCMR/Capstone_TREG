...
y_vec_R=reshape(x_all, length(x_all)*3, 1)
figure
...
Support=[7 10 13]; %The Support Polygon Base
...
t_move_1=.5;
Movement_i=3;
A_move{Movement_i}=[C_Move; M_Mat];
A_lock{Movement_i}=C_Lock;
b_move{Movement_i}=[b_con; 0; 1; .5]; 
b_lock{Movement_i}=zeros(size(A_lock{Movement_i},1),1);
t_move{Movement_i}=1.5;
